...
void sort_pairs(void)
{
    ...
    {
        int maxStrength = preferences[pairs[i].winner][pairs[i].loser];
        int maxIndex = i;
        for (int j = i + 1; j < pair_count; j++)
        {
            int currStrength = preferences[pairs[j].winner][pairs[j].loser];
...
bool is_cycle(int currNode, int target)
{
    if (currNode == target)
    {
        return true;
...
void print_winner(void)
{
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        if (countNotLocked == candidate_count)
        {
            printf("%s\n", candidates[i]);
        }
    }
